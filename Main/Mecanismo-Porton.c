...
{
    ESP_LOGI(tag, "Timer expired: ambos fines de carrera activos durante >%d ms", interval);
    timer_expirado = true;
}

static esp_err_t set_timer(void)
{
    if (xTimers == NULL) {
    ...
    // Reinicia el timer (lo pone en estado activo, con el periodo original)
    if (xTimerReset(xTimers, 0) != pdPASS) {
        ESP_LOGE(tag, "Timer could not be reset/started");
        return ESP_FAIL;
    }
...
int Func_Estado_Inicio(void)
{
    ...
    ESP_LOGI(tag, "Estado actual: Inicio");
    return 0;
}
...
int Func_Estado_Abierto(void)
{
    ...
    ESP_LOGI(tag, "Estado actual: Abierto");
    return 0;
}
...
int Func_Estado_Cerrado(void)
{
    ...
    ESP_LOGI(tag, "Estado actual: Cerrado");
    return 0;
}
...
int Func_Estado_Abriendo(void)
{
    ...
    ESP_LOGI(tag, "Estado actual: Abriendo");
    return 0;
}
...
int Func_Estado_Cerrando(void)
{
    ...
    ESP_LOGI(tag, "Estado actual: Cerrando");
    return 0;
}
...
int Func_Estado_Stop(void)
{
    ...
    ESP_LOGI(tag, "Estado actual: Stop");
    return 0;
}
...
int Func_Estado_Error(void)
{
    ...
    // Si el timer no está corriendo, lo arrancamos
    ...
    // Verificar si algún sensor se ha desactivado
    if (!io.fcc || !io.fca) {
        // Se liberó al menos uno → salimos del error
        if (timer_running && xTimers != NULL) {
    ...
    else if (timer_expirado) {
        // El timer expiró y ambos sensores siguen activos
        ESP_LOGW(tag, "Error persistente: ambos fines de carrera activos por más de %d ms", interval);
        // No cambiamos de estado, seguimos en Error
    }

    esp_mqtt_client_publish(mqtt_client, "/garage/estado", "Error", 0, 1, 0);
    ESP_LOGI(tag, "Estado actual: Error");
    return 0;
}
...
void app_main(void)
{
    ...
    // Crear el timer de error una sola vez
    xTimers = xTimerCreate(
        ...
        vTimerCallback);
    if (xTimers == NULL) {
        ESP_LOGE(tag, "Error timer could not be created");
    }
    ...
    while (1)
    {
        ...
        // Detección de error por ambos fines de carrera activos
